...
function Server() {
	...
	let [serverinput, setServerInput] = useState("");

	
	let [server, setmessage] = useState("");
	useEffect(async ()=>{
		let res = await axios({
			method: "GET",
			mod:"cots",
			url: "http://localhost:8000/server",
			// "Access-Control-Allow-Origin":  "http://192.168.1.115:3000",
			// "Access-Control-Allow-Methods": "GET",
			// "Access-Control-Allow-Headers": "Content-Type, Authorization",
			
		});
		setmessage(res.data[0].test)
		// message=res.data[0].test
	...
	const submitInput =async ()=>
	{
		...
		console.log(+serverinput)
		if (+serverinput > 65535 || +serverinput<0)
		{

			alert("Error")
		}
		else{
			let res=await axios.post('http://localhost:8000/server',{serverinput});
		}
